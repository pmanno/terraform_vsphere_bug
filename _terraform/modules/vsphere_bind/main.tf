...
resource "vsphere_virtual_machine" "vm" {
  ...
  custom_attributes = {
    ...
    "${data.vsphere_custom_attribute.ansible_group_0_attribute.id}" = "bind"
  ...
  clone {
    ...
    customize {
      dns_server_list = var.INSTANCE_DNS_SERVER_IPS
      network_interface {
        ipv4_address = var.INSTANCE_IP_ADDRESS
        ipv4_netmask = var.INSTANCE_IP_NETMASK
      }
      ipv4_gateway = var.INSTANCE_IP_GATEWAY
      linux_options {
...
resource "local_file" "ansible_inventory" {
  ...
  filename  = "${path.module}/../../../_ansible/playbooks/inventory_bind_${var.ENVIRONMENT_TARGET}"

  provisioner "local-exec" {
    command = "cd ${path.module}/../../../_ansible/playbooks; ansible-playbook -i inventory_bind_${var.ENVIRONMENT_TARGET} bind.yml -e environment_target=${var.ENVIRONMENT_TARGET} -e dns_servers=${join(",",var.INSTANCE_DNS_SERVER_NAMES)} -e provision=true"
  }
}